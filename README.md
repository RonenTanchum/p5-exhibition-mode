...
When `randomHash` is enabled, each playlist load receives a new 128-bit `?hash=` value. `hashIntervalValue` and `hashIntervalUnit` can also refresh the current playlist URL with a new hash before the URL itself changes.
...
```txt
https://art.phenomenalabs.com/ClassicalRevival/index.html?hash=42f7c74df0506a60d6b2e1c9a84f3b11
```
